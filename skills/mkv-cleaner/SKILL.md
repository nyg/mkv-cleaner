---
name: mkv-cleaner
description: Cleans MKV and M4V video files by remuxing them with mkvmerge, never re-encoding. Keeps the single best audio track in the target language (TrueHD Atmos > DTS-HD MA > EAC3 > AC3 > AAC), drops commentary, foreign audio, attachments and metadata junk, keeps all target-language subtitles and chapters, and renames the output to Title.Year.mkv or Series.SxxExx.Episode.mkv in a cleaned/ subfolder with a per-file markdown report. Use when the user asks to clean, strip, shrink, trim or remux MKV, M4V or video files, remove unwanted audio or subtitle tracks, or mentions mkvmerge, mkvtoolnix or remuxing.
license: MIT
